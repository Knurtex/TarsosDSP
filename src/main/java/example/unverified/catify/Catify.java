...
import dsp.WaveformSimilarityBasedOverlapAdd;
import dsp.WaveformSimilarityBasedOverlapAdd.Parameters;
import dsp.io.jvm.AudioDispatcherFactory;
import dsp.resample.RateTransposer;
import example.cli.SharedCommandLineUtilities;
import example.unverified.PitchShiftingExample;
...
public class Catify {
	...
	public static void main(String[] args) throws InvalidMidiDataException, IOException, UnsupportedAudioFileException, LineUnavailableException {
		try {
			if (args.length == 0) {
				...
				String resource = "/jingle_bells.mid";
	...
	public void buildSamples(){
		...
		if(sampleDirectory==null){
			...
			String resource = "/4915__noisecollector__cat3_mod.wav";
			copyFileFromJar(resource,path);
	...
	private void generateSound() throws UnsupportedAudioFileException, IOException, LineUnavailableException{
		...
		for(final MidiNoteInfo s: processedSamples){
          ...
          Parameters.musicDefaults(durationFactor,sampleRate));
			final AudioDispatcher
          dispatcher = AudioDispatcherFactory.fromFile(cs.getFile(),wsola.getInputBufferSize(), wsola.getOverlap());
