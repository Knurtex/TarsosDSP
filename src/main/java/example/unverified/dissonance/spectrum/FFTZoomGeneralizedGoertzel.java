package example.unverified.dissonance.spectrum;
...
import dsp.ui.Axis;
import dsp.ui.AxisUnit;
import dsp.ui.CoordinateSystem;
import dsp.ui.LinkedPanel;
import dsp.ui.ViewPort;
import dsp.ui.ViewPort.ViewPortChangedListener;
import dsp.ui.layers.AmplitudeAxisLayer;
import dsp.ui.layers.BackgroundLayer;
import dsp.ui.layers.DragMouseListenerLayer;
import dsp.ui.layers.LegendLayer;
import dsp.ui.layers.MouseCursorLayer;
import dsp.ui.layers.PitchContourLayer;
import dsp.ui.layers.Scalogram;
import dsp.ui.layers.SelectionLayer;
import dsp.ui.layers.TimeAxisLayer;
import dsp.ui.layers.TooltipLayer;
import dsp.ui.layers.VerticalFrequencyAxisLayer;
import dsp.ui.layers.WaveFormLayer;
import dsp.ui.layers.ZoomMouseListenerLayer;
import example.unverified.constantq.Player;
...
public class FFTZoomGeneralizedGoertzel extends JPanel{
		...
		player.addPropertyChangeListener(new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent arg0) {
				if(arg0.getPropertyName()=="state"){
					Player.PlayerState newState = (Player.PlayerState) arg0.getNewValue();
	...
	private void reactToPlayerState(Player.PlayerState newState){
		positionSlider.setEnabled(newState != Player.PlayerState.NO_FILE_LOADED);
		if(newState == Player.PlayerState.STOPPED || newState == Player.PlayerState.FILE_LOADED){
			newPositionValue = 0;
			positionSlider.setValue(0);
			setProgressLabelText(0, player.getDurationInSeconds());
		}
		if(newState == Player.PlayerState.FILE_LOADED){
		...
		//	timeFrequencyPane.addLayer(new GeneralizedGoertzelLayer(timeFrequencyPaneCS,player.getLoadedFile(),20));
			...
			Scalogram
          fftLayer = new Scalogram(timeFrequencyPaneCS,player.getLoadedFile().getAbsolutePath());
			...
			public void stateChanged(ChangeEvent arg0) {
				if (newPositionValue != positionSlider.getValue()) {
					...
					if (positionSlider.getValueIsAdjusting()) {
						...
						if(currentState == Player.PlayerState.PLAYING){
							player.play();							
						} 
					}
				}
		...
		chooseFileButton.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
	            ...
	            if (returnVal == JFileChooser.APPROVE_OPTION) {
	                ...
	                Player.PlayerState currentState = player.getState();
	                player.load(file);
	                if(currentState == Player.PlayerState.NO_FILE_LOADED || currentState == Player.PlayerState.PLAYING){
	                	player.play();
	                }
	            } else {
	                //canceled
		...
		player.addPropertyChangeListener(new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent evt) {
				if(evt.getPropertyName()=="state"){
					Player.PlayerState newState = (Player.PlayerState) evt.getNewValue();
					playButton.setEnabled(newState != Player.PlayerState.PLAYING && newState != Player.PlayerState.NO_FILE_LOADED);
					pauzeButton.setEnabled(newState == Player.PlayerState.PLAYING && newState != Player.PlayerState.NO_FILE_LOADED);
					stopButton.setEnabled((newState == Player.PlayerState.PLAYING || newState == Player.PlayerState.PAUZED) && newState != Player.PlayerState.NO_FILE_LOADED);
				}
			}
		});
